...
class TaskTagServiceImpl(BaseService[Any]):
    ...
    async def get(self, item_id: Tuple[UUID, UUID]) -> Any | None:
        item = await self.store.task_tags.get_by_id(item_id)
        return item

    async def create(self, data: dict) -> Any:
        item = await self.store.task_tags.create(data)
        return item

    async def update(self, item_id: Tuple[UUID, UUID], data: dict) -> Any:
        item = await self.store.task_tags.update(item_id, data)
        return item

    async def delete(self, item_id: Tuple[UUID, UUID]) -> None:
        await self.store.task_tags.delete(item_id)

    async def get_all(self) -> list[Any]:
        items = await self.store.task_tags.get_all()
        return items
