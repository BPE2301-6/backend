...
@router.patch("/me", summary="Обновить текущего пользователя",
...
async def list_users(search: str = "", limit: int = 20, offset: int = 0,
                     service_manager=Depends(get_service_manager)):
    users, total = await service_manager.user_service().get_list(search=search, limit=limit, offset=offset)
    return {
        "items": users,
        "total": total,
        "limit": limit,
